 ...
 return applicants ? applicants.length===0 ? (
    ...
    <div>
        <table className="w-full max-w-4xl bg-white border border-gray-200 max-sm:text-sm">
            ...
            <tbody>
                {applicants.filter(item=>item.jobId && item.userId).map((applicant,index)=>(<tr key={index} className="text-gray-700">
                    <td className="py-2 px-4 border-b text-center">{index+1}</td>
                    <td className="py-2 px-4 border-b text-center flex items-center"><img className="w-10 h-10 rounded-full mr-3 max-sm:hidden" src={applicant.userId.image}/>
                    ...
                    <td className="py-2 px-4 border-b text-center">
                        <a 
                        className="bg-cyan-50 text-cyan-400 px-3 py-1 rounded inline-flex gap-2 items-center"
                        href={applicant.userId.resume} target="_blank">Resume<img src={assets.resume_download_icon}/></a>
                    </td>
